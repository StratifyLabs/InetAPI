...
public:
  ...
  bool http_client_case() {
...

    {
...

      {
        ...
        printer().key("serverResponse",
                      StringView(response.data().add_null_terminator()));
        TEST_ASSERT(StringView(response.data().add_null_terminator()) ==
                    "Hello World");
      }

      {
        ...
        TEST_ASSERT(http_client
                        .post("index.html",
                              Http::Post().set_request(&request).set_response(
                                  &response))
                        .is_success());

        printer().key("request",
                      View(request.data()).to_string<GeneralString>());
        printer().key("response",
...

    {
...

      {
        ...
        TEST_EXPECT(
            http_client
                .put("/put", Http::ExecuteMethod().set_response(&response))
                .is_success());
        printer().key("response", response.data().add_null_terminator());
      }
    }
  ...
  bool socket_address_case() {
    ...
    TEST_ASSERT(SocketAddress4().set_protocol(S::Protocol::raw).protocol() ==
                S::Protocol::raw);

    TEST_ASSERT(SocketAddress4().set_protocol(S::Protocol::ip).protocol() ==
                S::Protocol::ip);

    TEST_ASSERT(SocketAddress4().set_protocol(S::Protocol::udp).protocol() ==
                S::Protocol::udp);

    TEST_ASSERT(SocketAddress4().set_protocol(S::Protocol::tcp).protocol() ==
                S::Protocol::tcp);

    TEST_ASSERT(SocketAddress4()
                    .set_address(IpAddress4("1.0.0.127"))
                    .get_address_string() == "1.0.0.127");

    TEST_ASSERT(
        SocketAddress4().set_address(IpAddress4(0)).get_address_string() ==
        "0.0.0.0");
    printer().key("address", SocketAddress4()
                                 .set_address(IpAddress4(0x12345678))
                                 .get_address_string()
                                 .string_view());
    TEST_ASSERT(SocketAddress4()
                    .set_address(IpAddress4(0x12345678))
                    .get_address_string() == "18.52.86.120");

    TEST_ASSERT(SocketAddress4().set_port(3000).port() == 3000);
    TEST_ASSERT(SocketAddress6().set_port(3000).port() == 3000);

    TEST_ASSERT(SocketAddress6().set_protocol(S::Protocol::raw).protocol() ==
                S::Protocol::raw);

    TEST_ASSERT(SocketAddress6().set_protocol(S::Protocol::ip).protocol() ==
                S::Protocol::ip);

    TEST_ASSERT(SocketAddress6().set_protocol(S::Protocol::udp).protocol() ==
                S::Protocol::udp);

    TEST_ASSERT(SocketAddress6().set_protocol(S::Protocol::tcp).protocol() ==
                S::Protocol::tcp);

    printer().key("address6",
                  SocketAddress6()
                      .set_address(IpAddress6("12:34:56:78:ab:cd:ef:01"))
                      .get_address_string()
                      .string_view());
    TEST_ASSERT(SocketAddress6()
                    .set_address(IpAddress6("12:34:56:78:ab:cd:ef:01"))
                    .get_address_string() ==
    ...
    Socket server = std::move(
        Socket(server_address.family(), server_address.type())
            .set_option(SocketOption(
                S::Level::socket, S::NameFlags::socket_reuse_address, true)));
  ...
  bool socket_tcp_reflection_case(Socket::Family family) {
    ...
    PRINTER_TRACE(printer(),
                  "incoming " +
                      String(View(incoming_data).truncate(return_value())));
  ...
  static bool socket_udp_server(UnitTest *self) {
    ...
    auto bind_socket = [self]() {
      ...
      API_RETURN_VALUE_ASSIGN_ERROR(std::move(Socket(self->m_family)), "",
                                    EINVAL);
    };
    ...
    if (is_success()) {
      ...
      server_socket.send_to(client_address,
  ...
  bool socket_udp_reflection_case(Socket::Family family) {
    ...
    TEST_ASSERT(address_info.list().count() > 0);
    Socket socket = std::move(
        Socket(address_info.list().at(0))
            .set_option(SocketOption(
                S::Level::socket, S::NameFlags::socket_reuse_address, true)));
